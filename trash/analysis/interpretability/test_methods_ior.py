...
class InterpretabilityTester:
    ...
    def load_ior_job(self, csv_path: str) -> torch.Tensor:
        """Load IOR job features from CSV file"""
        import pandas as pd
        
        logger.info(f"Loading IOR job from {csv_path}")
        df = pd.read_csv(csv_path)
        
        # Get feature columns (exclude metadata)
        feature_cols = [col for col in df.columns 
                       if col not in ['performance', 'jobid', 'nprocs_actual', 
                                     'performance_raw_mbs', 'runtime', 'tag']]
        
        # Take first row (aggregated job)
        job_features = df.iloc[0][feature_cols].values.astype(np.float32)
        
        # Convert to tensor
        job_tensor = torch.tensor(job_features, dtype=self.dtype)
        
        # Check if we need to pad features to match model
        if hasattr(self, 'checkpoint_features') and self.checkpoint_features > len(job_features):
            logger.info(f"Padding IOR features from {len(job_features)} to {self.checkpoint_features}")
            padding = torch.zeros(self.checkpoint_features - len(job_features), dtype=self.dtype)
            job_tensor = torch.cat([job_tensor, padding])
        
        logger.info(f"Loaded IOR job with {job_tensor.shape[0]} features")
        
        # Get actual performance if available
        if 'performance' in df.columns:
            self.ior_actual_performance = df.iloc[0]['performance']
            logger.info(f"Actual performance: {self.ior_actual_performance:.4f}")
        
        return job_tensor.to(self.device)

    def create_ior_subgraph(self, ior_features: torch.Tensor, k_neighbors: int = 50):
        """Create subgraph for IOR job by finding similar nodes"""
        from torch_geometric.data import Data
        from sklearn.metrics.pairwise import cosine_similarity
        
        logger.info(f"Finding {k_neighbors} nearest neighbors for IOR job...")
        
        # Compute similarities with all nodes
        ior_np = ior_features.cpu().numpy().reshape(1, -1)
        all_features_np = self.data.x.cpu().numpy()
        
        # Ensure same number of features
        if ior_np.shape[1] != all_features_np.shape[1]:
            min_features = min(ior_np.shape[1], all_features_np.shape[1])
            ior_np = ior_np[:, :min_features]
            all_features_np = all_features_np[:, :min_features]
        
        similarities = cosine_similarity(ior_np, all_features_np).squeeze()
        
        # Get top-k similar nodes
        top_k_indices = np.argsort(similarities)[-k_neighbors:][::-1]
        top_k_similarities = similarities[top_k_indices]
        
        logger.info(f"Similarity range: {top_k_similarities.min():.4f} - {top_k_similarities.max():.4f}")
        
        # Build subgraph with IOR as node 0 and neighbors
        num_nodes = k_neighbors + 1
        
        # Features: IOR job first, then neighbors
        # Make a copy to avoid negative stride issue
        neighbor_features = self.data.x[top_k_indices].clone()  # Use clone() instead of direct indexing
        
        x = torch.cat([
            ior_features.unsqueeze(0),
            neighbor_features
        ], dim=0)
        
        # Build edges (connect IOR to all neighbors)
        edge_list = []
        edge_weights = []
        
        for i, (neighbor_idx, sim) in enumerate(zip(top_k_indices, top_k_similarities)):
            # IOR (node 0) to neighbor (node i+1)
            edge_list.append([0, i+1])
            edge_list.append([i+1, 0])  # bidirectional
            edge_weights.extend([sim, sim])
        
        # Also add edges between similar neighbors
        # Make a copy to avoid stride issues
        neighbor_features_np = all_features_np[top_k_indices].copy()
        neighbor_sims = cosine_similarity(neighbor_features_np)
        for i in range(k_neighbors):
            for j in range(i+1, k_neighbors):
                if neighbor_sims[i, j] > 0.8:  # High similarity threshold
                    edge_list.append([i+1, j+1])
                    edge_list.append([j+1, i+1])
                    edge_weights.extend([neighbor_sims[i, j], neighbor_sims[i, j]])
        
        edge_index = torch.tensor(edge_list, dtype=torch.long).t()
        edge_attr = torch.tensor(edge_weights, dtype=self.dtype).unsqueeze(1)
        ...
        logger.info(f"Created IOR subgraph: {num_nodes} nodes, {edge_index.shape[1]} edges")
        
        # Store neighbor info for analysis
        self.ior_neighbors = top_k_indices
        self.ior_similarities = top_k_similarities
        
        return subgraph.to(self.device), 0  # IOR node is always index 0
    ...
    def analyze_ior_job(self, csv_path: str, k_neighbors: int = 50) -> Dict:
        """Analyze IOR job with all interpretability methods"""
        logger.info(f"\n{'='*60}")
        logger.info(f"Analyzing IOR Job from {csv_path}")
        logger.info(f"{'='*60}")
        
        # Load IOR features
        ior_features = self.load_ior_job(csv_path)
        
        # Create subgraph
        subgraph, mapping = self.create_ior_subgraph(ior_features, k_neighbors)
        
        results = {
            'node_idx': 'IOR_job',
            'performance': self.ior_actual_performance if hasattr(self, 'ior_actual_performance') else None,
            'methods': {}
        }
        
        # Show neighbor statistics
        if hasattr(self, 'ior_neighbors'):
            neighbor_perfs = self.data.y[self.ior_neighbors].cpu().numpy()
            logger.info(f"Neighbor performance statistics:")
            logger.info(f"  Mean: {neighbor_perfs.mean():.4f}")
            logger.info(f"  Std:  {neighbor_perfs.std():.4f}")
            logger.info(f"  Min:  {neighbor_perfs.min():.4f}")
            logger.info(f"  Max:  {neighbor_perfs.max():.4f}")
            
            results['neighbor_stats'] = {
                'mean': float(neighbor_perfs.mean()),
                'std': float(neighbor_perfs.std()),
                'min': float(neighbor_perfs.min()),
                'max': float(neighbor_perfs.max())
            }
        ...
        # 4. Consensus
        logger.info(f"Finding consensus...")
        consensus = self.find_consensus(results['methods'], min_methods=2)
        results['consensus'] = consensus
        
        if consensus:
            logger.info("\n  CONSENSUS BOTTLENECKS (identified by multiple methods):")
...
def main():
    ...
    parser.add_argument('--ior-csv', type=str,
                       help='Path to IOR job CSV file to analyze')
    parser.add_argument('--ior-neighbors', type=int, default=50,
                       help='Number of neighbors for IOR analysis')
    ...
    if args.visualize_node:
        ...
        # Analyze IOR job
        logger.info("Running IOR job analysis")
        results = tester.analyze_ior_job(
            csv_path=args.ior_csv,
            k_neighbors=args.ior_neighbors
        )
        
        # Save results
        output_path = Path('./ior_interpretability_results.json')
        with open(output_path, 'w') as f:
